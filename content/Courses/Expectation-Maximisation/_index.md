...
<p align="center">
<img src="https://github.com/tbonnair/academic-online-cv/blob/master/content/Courses/Expectation-Maximisation/gaussian_mixtures.png?raw=true" alt="fig:GMM" width="500"/>
</p>
...
<p align="center">
<img src="https://github.com/tbonnair/academic-online-cv/blob/master/content/Courses/Expectation-Maximisation/EM_climbing.png?raw=true" alt="fig:EM" width="600"/>
</p>
...
Well, in this scenario, we can derive, in the E-step, the probability of the latent variables given the observed data and the current parameters of the model. This is what we call _responsibilities_ and we will note them $p_{ik} = p(z_i = k | \boldsymbol{x}_i, \boldsymbol{\Theta}^{(t)})$. They represent the probability of a datapoint $i$ being generated by the $k$th component of the model. Using Bayes' theorem,
$$
  p_{ik} = \frac{\pi_k \, f(\boldsymbol{x}_i, \boldsymbol{\theta}_k^{(t)})}{\sum_{j=1}^K \pi_j \, f(\boldsymbol{x}_i, \boldsymbol{\theta}_j^{(t)})}.
$$
...
Each of them can be readily expressed. The first one is nothing else than the normal probability of the $k^\mathrm{th}$ component, so $\mathcal{N}(\boldsymbol{x}_{i}, \boldsymbol{\theta}_{k} ) $ . The second one is just the amplitude of the component $k$ in the model, i.e. $\pi_{k}$. The denominator is the full probability of a datapoint being generated by the model so $\sum_{j=1}^K \pi_j \mathcal{N}(\boldsymbol{x}_i, \boldsymbol{\theta}_j) $.This latter can also be seen as the normalisation of the probability distribution over latent variables.
{style="color: red"}
{{< /spoiler >}}
