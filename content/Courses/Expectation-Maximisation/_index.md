...
Each of them can be readily expressed. The first one is nothing else than the normal probability of the $k^\mathrm{th}$ component, that is $ \mathcal{N}(\boldsymbol{x}_i, \boldsymbol{\theta}_k) $ . The second one is the amplitude of the component $k$ in the model, i.e. $\pi_{k}$. The denominator is the full probability of a datapoint being generated by the model so $\sum_{j=1}^K \pi_j \mathcal{N}(\boldsymbol{x}_i, \boldsymbol{\theta}_j) $.This latter can also be seen as the normalisation of the probability distribution over latent variables.
{style="color: red"}
{{< /spoiler >}}
