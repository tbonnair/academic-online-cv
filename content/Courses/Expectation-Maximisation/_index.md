...
Each of them can be readily expressed. The first one is nothing else than the normal probability of component $k$, $\mathcal{N}(\boldsymbol{x}_i, \boldsymbol{\theta}_k ) $. The second one is just the amplitude of the component $k$ in the model, i.e. $\pi_k$. The denominator is the full probability of a datapoint being generated by the model so $\sum_{j=1}^K \pi_j \mathcal{N}(\boldsymbol{x}_i, \boldsymbol{\theta}_j) $. This latter can also be seen as the normalisation of the probability distribution over latent variables.
{style="color: red"}
{{< /spoiler >}}
